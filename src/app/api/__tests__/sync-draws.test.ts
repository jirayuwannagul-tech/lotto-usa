...
describe("getNextDrawDates", () => {
  it("uses DST-aware Los Angeles time in January for Powerball", () => {
    ...
    expect(result?.drawDate.toISOString()).toBe("2025-01-07T06:59:00.000Z")
  ...
  it("uses DST-aware Los Angeles time in June for Mega Millions", () => {
    ...
    expect(result).not.toBeNull()
    expect(result?.drawDate.toISOString()).toBe("2025-06-04T06:00:00.000Z")
  ...
  it("keeps local Los Angeles wall-clock times stable across seasons", () => {
    ...
    expect(formatInTimeZone(winter!.drawDate, "America/Los_Angeles", "HH:mm")).toBe("22:59")
    expect(formatInTimeZone(summer!.drawDate, "America/Los_Angeles", "HH:mm")).toBe("22:59")
  })
})
