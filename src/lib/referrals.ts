...
export async function appointReferrer(userId: string, name: string) {
  ...
  for (let attempt = 0; attempt < 10; attempt++) {
    const referralCode = createReferralCodeFromName(name)
    try {
      return await prisma.referrerProfile.create({ data: { userId, referralCode } })
    } catch (e) {
      // P2002 = unique constraint — try a new code
